...
npm run db:link             # supabase link --project-ref jywwgoceybjendvpbqea
npm run db:push             # apply all migrations to the linked remote
...
```bash
npm run db:diff <change_name>   # captures remote diff into a migration file
```
...
```bash
npm install
npm run dev -w web   # http://localhost:8080
```
